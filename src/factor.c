  ...
  for (int i = 0; i < LENGTH(flist); i++) {
    ...
    if (isNull(xattr)) {
      ...
      if (isMatrix(xattr)) {
        if (nrows(xattr) != len) {
          error(
              "Factor interaction terms (%d) must have the same length (%d) as "
              "the factor",
              LENGTH(xattr), len);
        }
        truefac--;
...
static int Components(int **vertices, FACTOR **factors, int K) {
  ...
  stack = (int*)Calloc(numvert * 4, int);
...
#define POPALL      \
  ...
  do {
   ...
   If it's already marked, go to next datapoint for the vertex (incidence
   matrix) If data-points are exhausted, go to next factor, start over with
   datapoints. If factors are exhausted, pop the stack If final stack-frame,
   we're done with component.
      */
    ...
    /* Find next component */
    while (candvert < factors[0]->nlevels && vertices[0][candvert] != 0)
      candvert++;
    curcomp++;
...
// Algorithm from:
// "A Note on the Determination of Connectedness in an N-Way Cross
// Classification" D.L. Weeks and D.R. Williams, Technometrics, vol 6 no 3,
// August 1964 There probably exists faster algorithms, this one is quite slow
static void wwcomp(FACTOR *factors[], int numfac, int N, int *newlevels) {
  int level = 0;
  int chead = 0;
  int *newlist = (int*)Calloc(N, int);
  int *oldlist = (int*)Calloc(N, int);
  int oldstart = 0;
  // For cache-efficiency, make a numfac x N matrix of the factors
  int *facmat = (int*)Calloc(numfac * N, int);
  ...
  while (oldstart < N) {
    ...
    for (int i = 0; i < newidx; i++) {
      ...
      // search for observations with distance 1 from newnode, mark them with
      // level
...
SEXP MY_wwcomp(SEXP flist) {
  ...
  // Now it's time to order the levels by decreasing size, so let's compute the
  // sizes
  int levels = 0;
  for (int i = 0; i < N; i++)
...
SEXP MY_conncomp(SEXP flist) {
  ...
  for (i = 0; i < numfac; i++) {
    ...
    f->nlevels = LENGTH(getAttrib(VECTOR_ELT(flist, i), R_LevelsSymbol));
    if (f->nlevels == 0)
      error("Factor %s has zero levels", CHAR(STRING_ELT(GET_NAMES(flist), i)));
    invertfactor(f, N);
  }
  ...
  */
  ...
  gpsiz = (double*)Calloc(comps, double);
  idx = (int*)Calloc(comps, int);
